...
func decorateHTML(_ html: String) -> String {
    ...
    <style> body { width: 96%; margin-bottom: -10px; } img { max-width: 100%; width: auto; height: auto; } iframe { max-width: 100%; } video { max-width: 100%; } </style>
        </head>
        <body>
            \(html)
        </body>
...
func decorateArticleHTML(_ article: ArticleInfo) -> String {
    ...
    <html>
        <head>
            <meta name="viewport"  content="width=device-width, initial-scale=1, maximum-scale=1"/>
            <style> body {  } img { max-width: 100%; width: auth; height: auto; } iframe { max-width: 100%; } video { max-width: 100% } </style>
        </head>
        <body>
            <h2>\(article.title)</h2>
            <hr />
